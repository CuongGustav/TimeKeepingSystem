...
url_cam = 'http://192.168.179.174/cam-hi.jpg'
url_server = 'https://servertimekeepingiot.onrender.com/send_data'
            ...
            while stream.isOpened():
                ...
                if (self.current_frame_face_cnt == self.last_frame_face_cnt) and (
                    ...
                    if self.current_frame_face_cnt == 0:
                        ...
                        # self.url là url của server nha
                        for id in self.current_frame_face_name_list:
                            if id != 'unknown' and len(self.current_frame_face_name_list) > 0:
                                ...
                                try:
                                    ...
                                    _, img_encoded = cv2.imencode('.jpg', img_rd)
                                    file = {'image': (f'{id}.jpg', img_encoded.tobytes(), 'image/jpeg')}
                                    response = requests.post(self.url, data=data, files=file)
                                    response.raise_for_status()
                                    print("đã gửi thành công :",id)
