...
url = 'http://192.168.179.174/cam-hi.jpg'
class Face_Register:
    def __init__(self):
        person_num_list_id = []
        if os.listdir("data/data_faces_from_camera/"):
            ...
            for person in person_list:
                person_num_list_id.append(person)

            # for person in person_list:
            #     person_order = person.split('_')[1].split('_')[0]
            #     person_num_list.append(int(person_order))
            for i in range(len(person_list)):
                person_num_list.append(i+1)
        ...
        self.input_name = ttk.Combobox(self.frame_right_info, values=person_num_list_id, state='readonly', width=45)
    ...
    def GUI_info(self):
        ...
        tk.Label(self.frame_right_info,
                 ...
                 text="Select Employee:").grid(row=7, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)
    ...
    def save_current_face(self):
        name_folder = self.input_name.get()
        if name_folder != "":
            count_images = 0
            if os.listdir(f"data/data_faces_from_camera/{name_folder}/"):
                images = os.listdir(f"data/data_faces_from_camera/{name_folder}/")
            ...
            if self.current_frame_faces_cnt == 1:
                if not self.out_of_range_flag:
                    ...
                    self.log_all["text"] = "\"" + self.current_face_dir + "/img_face_" + str(
                        count_images) + ".jpg\"" + " saved!"
                    self.face_ROI_image = cv2.cvtColor(self.face_ROI_image, cv2.COLOR_BGR2RGB)

                    cv2.imwrite(self.current_face_dir + "/img_face_" + str(count_images) + ".jpg", self.face_ROI_image)
            ...
            self.log_all["text"] = "Please choose employee who you want to save image"
